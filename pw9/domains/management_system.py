...
class ManagementSystem:
    ...
    def save_students(self):
        threading.Thread(target=self._save_students).start()
    ...
    def save_courses(self):
        threading.Thread(target=self._save_courses).start()
    ...
    def save_marks(self):
        threading.Thread(target=self._save_marks).start()

    def _save_marks(self):
        marks_data = {student.get_id(): student._marks for student in self._students}
        with open("marks.pkl", "wb") as file:
            pickle.dump(marks_data, file)

    def compress_data(self):
        threading.Thread(target=self._compress_data).start()
